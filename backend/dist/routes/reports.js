...
router.get('/profitability', requireAuth, async (_req, res) => {
    try {
        ...
        const profitability = recipes.map(recipe => {
            ...
            const preparations = recipe.ingredients.length > 0
                ? Math.floor(Math.min(...recipe.ingredients.map(ing => {
...
router.get('/analytics', requireAuth, async (req, res) => {
    try {
        ...
        Object.keys(salesByDate).forEach(date => {
            if (!date)
                return;
            const dateConsumptions = recipeConsumptions.filter(c => c.createdAt.toISOString().split('T')[0] === date);
