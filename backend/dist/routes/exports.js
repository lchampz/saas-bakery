...
router.get('/products/csv', requireAuth, async (req, res, next) => {
    try {
        ...
        const csvRows = products.map(p => `"${p.name}",${p.quantity},${p.unit},${p.pricePerGram || 0},${p.minLevel || 0}`).join('\n');
...
router.get('/recipes/csv', requireAuth, async (req, res, next) => {
    try {
        ...
        const csvRows = recipes.map(recipe => {
            const ingredients = recipe.ingredients.map(ing => `${ing.product.name} (${ing.amount}${ing.product.unit})`).join('; ');
...
router.get('/purchases/csv', requireAuth, async (req, res, next) => {
    try {
        ...
        const csvRows = purchases.map(purchase => {
            const items = purchase.items.map(item => `${item.product.name} (${item.quantity}${item.product.unit})`).join('; ');
...
router.get('/stock-report/pdf', requireAuth, async (req, res, next) => {
    try {
        const products = await prisma.product.findMany({
            where: { deletedAt: null },
            orderBy: { name: 'asc' }
