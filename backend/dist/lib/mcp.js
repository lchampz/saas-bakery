...
Estrutura JSON obrigatória (retorne exatamente assim):
{
    ...
    parseRecipeResponse(recipe, context) {
        ...
        if (estimatedCost === 0) {
            for (const ing of ingredients) {
                const product = context.availableProducts.find(p => p.name.toLowerCase() === ing.productName.toLowerCase());
                if (product?.pricePerGram) {
                    // Converter para gramas se necessário
                    let amountInGrams = ing.amount;
                    if (ing.unit === 'kg')
                        amountInGrams = ing.amount * 1000;
                    if (ing.unit === 'mg')
                        amountInGrams = ing.amount / 1000;
    ...
    generateRecipeFallback(context) {
