...
router.post('/generate', requireAuth, async (req: Request, res: Response, next: NextFunction) => {
	try {
		const { description, availableProducts, servingSize, dietaryRestrictions } = req.body as {
			description: string;
		...
		if (availableProducts && availableProducts.length > 0) {
			filteredProducts = products.filter(p => 
				availableProducts.some(ap => p.name.toLowerCase().includes(ap.toLowerCase()))
			);
		}
		...
		try {
			...
			return next(new CustomError(
				error instanceof Error ? error.message : 'Erro ao gerar receita com IA. Verifique se o Ollama está rodando e se o modelo está disponível.',
				500
			));
		}
		...
		try {
			...
			const ingredientsWithIds = await Promise.all(
				generatedRecipe.ingredients.map(async (ing) => {
					...
					if (!product) {
						...
						const similar = filteredProducts.find(p => 
			...
			const validIngredients = ingredientsWithIds.filter((ing): ing is NonNullable<typeof ing> => ing !== null);
...
router.post('/create', requireAuth, async (req: Request, res: Response, next: NextFunction) => {
	try {
		const { name, ingredients, instructions, servingSize, estimatedCost } = req.body as {
			name: string;
